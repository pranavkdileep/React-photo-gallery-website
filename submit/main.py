...
def get_prompt():
    url = "https://gallery-api-y2pm.onrender.com/getprompt"
...
def del_image(image_id):
    url = "https://gallery-api-y2pm.onrender.com/del_image/" + str(image_id)
    response = requests.get(url)
    response_json = response.json()
...
def upload_db(image_link, image_id, prompt, negative_prompt, height, width):
    url = "https://gallery-api-y2pm.onrender.com/upload_db"
    data = {
